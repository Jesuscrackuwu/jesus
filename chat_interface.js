...
class ChatInterface {
    ...
    init() {
        ...
        // Esto corrige el bug donde 100dvh - 56px puede ser incorrecto
        // si el navbar renderizado mide más o menos de 56px
        this._measureNavbarHeight();

        this._initVisualViewport();
        // Ajustar padding después de que el DOM esté listo
        if (document.readyState === 'complete') {
            this._adjustMessagesHeight();
        } else {
            window.addEventListener('load', () => this._adjustMessagesHeight());
        }
    }
